---
tags:
  - Special relativity
  - Galilei group
  - Minkowski spacetime
  - Lorentz transformation
  - Poincare transformation
  - Four-vector
  - Four-momentum
  - Rapidity
  - Relativistic fields
---

# Special Relativity I

<div class="document-meta">
  <div><span>Subject</span><strong>Special relativity</strong></div>
  <div><span>Course</span><strong>Cosmology</strong></div>
  <div><span>Lecturer</span><strong>Thomas Gajdosik</strong></div>
  <div><span>Level</span><strong>Master’s course</strong></div>
  <div><span>Length</span><strong>35 pages</strong></div>
  <div><span>Format</span><strong>Notes and slides</strong></div>
</div>

## Abstract

These notes develop the special-relativistic foundations used in cosmology.
They begin with Newtonian spacetime and Galilean transformations, introduce
the Galilei group and the postulates of special relativity, and then develop
affine spaces, vectors, tensors, Minkowski geometry, Lorentz and Poincaré
transformations, rapidity, four-momentum, and the transformation of
relativistic fields.

<div class="keyword-list" aria-label="Keywords">
  <span>Galilean relativity</span>
  <span>Galilei group</span>
  <span>Minkowski spacetime</span>
  <span>Lorentz transformations</span>
  <span>Poincaré transformations</span>
  <span>Four-vectors</span>
  <span>Four-momentum</span>
  <span>Rapidity</span>
  <span>Relativistic fields</span>
</div>

<div class="source-panel">
  <span class="source-panel__label">Source material</span>
  <h2>Thomas Gajdosik: Cosmology — 1. Special Relativity</h2>
  <p>Original lecture slides used as source material for these notes.</p>
</div>

<div class="resource-actions">
  <a class="resource-button resource-button--primary" href="https://darius-mathphys.github.io/math-physics-library/assets/pdfs/special_relativity_1_notes.pdf" target="_blank" rel="noopener">Open notes</a>
  <a class="resource-button" href="https://darius-mathphys.github.io/math-physics-library/assets/pdfs/special_relativity_1_notes.pdf" download>Download notes</a>
  <a class="resource-button" href="https://darius-mathphys.github.io/math-physics-library/assets/pdfs/special_relativity_1_slides.pdf" target="_blank" rel="noopener">Open original slides</a>
  <a class="resource-button" href="https://darius-mathphys.github.io/math-physics-library/assets/pdfs/special_relativity_1_slides.pdf" download>Download original slides</a>
</div>

## Document

<div class="pdf-shell">
  <object
    data="https://darius-mathphys.github.io/math-physics-library/assets/pdfs/special_relativity_1_notes.pdf#view=FitH"
    type="application/pdf"
    aria-label="Special Relativity I lecture notes"
  >
    <p>
      Your browser cannot display the embedded PDF.
      <a href="https://darius-mathphys.github.io/math-physics-library/assets/pdfs/special_relativity_1_notes.pdf">Open the notes in a new tab.</a>
    </p>
  </object>
</div>

