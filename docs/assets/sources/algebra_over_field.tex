...
We develop this definition from binary operations, through groups, rings,
fields, and vector spaces. Linear and bilinear maps then provide the language
needed to express the compatibility. The principal geometric examples are
algebras of smooth functions and their germs.
...
Axiom $(R2)$ says that $(A,\cdot)$ is a semigroup. Axiom $(R3)$ relates
multiplication to addition. Without commutativity of multiplication, neither
distributive law follows from the other, so both are stated.
...
\emph{unital} states this additional assumption. Some books include $(R4)$ in
the definition of a ring, so the convention should always be checked.
...
the nonzero elements and has identity $1_K$. The zero-product identities
extend these properties to all of $K$. Thus $K$ is a commutative unital ring
with $1_K\neq0_K$, and $(F2)$ supplies the inverses.
...
\]
It therefore does not define an associative algebra. It is an example of a
\emph{Lie algebra} product: it is bilinear, antisymmetric, and satisfies the
Jacobi identity
\[
...
Throughout these notes, \emph{algebra} means \emph{associative algebra}. A
multiplicative identity is not required. This is the convention in Tu,
\S2.2. In broader usage, an algebra may have a bilinear product that is not
associative, as the cross product illustrates.
...
\begin{precision}[The three conditions in Tu, \S2.2]
The vector-space assumption already supplies the abelian group $(A,+)$.
Condition (i) supplies $(A1)$. Condition (ii) supplies $(A2)$. Condition (iii)
supplies $(A3)$. Consequently, conditions (i) and (ii), together with the
additive vector-space structure, make $A$ a ring. Condition (iii) makes this
ring multiplication compatible with the scalar field $K$.
\end{precision}
...
associative. Its identity is $\operatorname{id}_V$.
If $\dim_K V=n\geq1$, choosing a basis identifies this algebra with $M_n(K)$.
\end{examplebox}
...
The algebraic structures above apply directly to the smooth functions used
in Tu's construction of tangent vectors.
...
As in Tu's convention, preservation of a multiplicative identity is an
additional condition. It does not follow from linearity and preservation of
products. For example, the zero map $\R\to\R$ has both properties but does
not preserve $1$.
